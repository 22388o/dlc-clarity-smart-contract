...
(define-public (open-new-dlc (uuid (buff 8)) (asset (buff 32)) (closing-time uint) (emergency-refund-time uint) (creator principal))
  (begin
    ...
    (nft-mint? open-dlc uuid .discreet-log-storage-v2-1))) ;;mint an open-dlc nft to keep track of open dlcs
...
(define-public (close-dlc (uuid (buff 8)) (outcome bool))
  (let 
    ...
    (print {
      uuid: uuid,
      outcome: outcome})
    (nft-burn? open-dlc uuid .discreet-log-storage-v2-1) ;;burn the open-dlc nft related to the UUID
      )) 
